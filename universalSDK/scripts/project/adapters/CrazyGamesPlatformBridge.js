...
export default class CrazyGamesPlatformBridge extends PlatformBridgeBase {
    ...
    async load() {
        if (!this._platformSdk) return {};
        try {
            const raw = this._platformSdk.data.getItem("save");
            if (raw) return JSON.parse(raw);
        } catch (e) { console.error("[CrazyGames] load error", e); }
        return {};
    }

    async save(data) {
        if (!this._platformSdk) return;
        try { this._platformSdk.data.setItem("save", JSON.stringify(data)); }
        catch (e) { console.error("[CrazyGames] save error", e); }
    }
