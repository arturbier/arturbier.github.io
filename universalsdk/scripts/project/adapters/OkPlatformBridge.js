...
export default class OkPlatformBridge extends VkPlatformBridge {
    ...
    async checkAdBlock() {
        const fapi = this._fapi || (typeof FAPI !== "undefined" ? FAPI : null);
        if (!fapi || !fapi.invokeUIMethod) return false;
        ...
        return new Promise((resolve) => {
            ...
            window.API_callback = (method, result, data) => {
                if (typeof prev === "function") { try { prev(method, result, data); } catch (e) { /* ignore */ } }
                if (done) return;
                if (method === "isAdBlockEnabled") {
                    done = true;
                    window.API_callback = prev;
                    resolve(result === "ok" && data === "true");
                }
            };
            setTimeout(() => { if (!done) { done = true; window.API_callback = prev; resolve(false); } }, 5000);
            try { fapi.invokeUIMethod("isAdBlockEnabled"); } catch (e) { done = true; window.API_callback = prev; resolve(false); }
        });
    }
}
