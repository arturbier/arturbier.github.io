...
import UsdkConfig from "./config.js";
...
window.usdk = {
    ...
    async _doInit() {
        ...
        // Pass Firebase config (shared by storage and leaderboard) to the adapter.
    ...
    async isSupported(feature) {
        ...
        const map = {
            ...
            leaderboards: "isLeaderboardsSupported"
        };
    ...
    // ---------- leaderboard ----------
    leaderboard: {
        async submitScore(score) { await window.usdk._ensureReady(); return window.usdk.adapter.submitScore(score); },
        async getTop(limit) { await window.usdk._ensureReady(); return window.usdk.adapter.getTop(limit); },
        async getPlayerRank() { await window.usdk._ensureReady(); return window.usdk.adapter.getPlayerRank(); },
        async show(limit) { await window.usdk._ensureReady(); return window.usdk.adapter.showLeaderboard(limit); }
    },
